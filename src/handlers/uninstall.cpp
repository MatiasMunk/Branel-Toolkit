...
namespace ActionHandlers::Uninstall
{
    ...
    void Actor(Action &action, std::array<intptr_t, 4> data_ptr)
    {
        ...
        std::thread uninstall(&UninstallSQL);
        uninstall.detach();
    }
    ...
    void UninstallSQL()
    {
        ...
        do
        {
            ...
            for(vector<Software>::iterator iter = list->begin(); iter!=list->end(); iter++)
            {
                ...
                if((iter->DisplayName.find(L"SQL Server") != std::string::npos))
                {
                    ...
                    if(ProgramManager::StartProcess(&uninstall_cmd[0]) == 0)
                    {
                        ...
                        application->modified_programs.push_back(uninstalled_program_name);
                    }
                    else
                    {
        ...
        GUI().OpenPopup("Result", application->modified_programs, "Uninstall");
